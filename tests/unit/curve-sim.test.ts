    ...
    const { orchestratorAddress } = await run({
      ...
      fundingManager: {
        bondingCurveParams: {
          ...
          initialIssuanceSupply: String(Number(maxSupply) / 5), // At least 200 Million at the time of graduation + the amount the users mint upon buy
          initialCollateralSupply: String(Number(maxSupply) / 10000),
          buyIsOpen: true,
          sellIsOpen: true,
        },
